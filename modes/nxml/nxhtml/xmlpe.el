...
;; Created: 2005-06-17
(defconst xmlpe:version "0.57") ;; Version:
;; Last-Updated: Wed Feb 21 01:15:57 2007 (3600 +0100)
;; Keywords: languages
...
(defun xmlpe--post-command()
  "Function for `post-command-hook'. Confine position and mark to edit area."
  (condition-case nil
      (progn
	(when (memq xmlpe-header-overlay (overlays-at (point)))
	  (goto-char (overlay-end xmlpe-header-overlay)))
	(when (memq xmlpe-footer-overlay (overlays-at (point)))
	  (goto-char (- (overlay-start xmlpe-footer-overlay) 1)))
	(when (>= (point) (overlay-end xmlpe-footer-overlay))
	  (goto-char (- (overlay-start xmlpe-footer-overlay) 1)))
        (when mark-active
          (when (memq xmlpe-header-overlay (overlays-at (mark)))
            (set-mark (overlay-end xmlpe-header-overlay)))
          (when (memq xmlpe-footer-overlay (overlays-at (mark)))
            (set-mark (- (overlay-start xmlpe-footer-overlay) 1)))
          (when (>= (mark) (overlay-end xmlpe-footer-overlay))
            (set-mark (- (overlay-start xmlpe-footer-overlay) 1))))
        )
    (error nil) ))
...
(defun xmlpe-set-css()
  (interactive)
  (let ((css (completing-read "CSS URL: " xmlpe-css-hrefs nil nil xmlpe-css-href 'xmlpe-set-css-hist "")))
    (setq xmlpe-css-href css))
  (xmlpe-update-header-hrefs))
